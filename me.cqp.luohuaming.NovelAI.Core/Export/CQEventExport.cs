...
using me.cqp.luohuaming.NovelAI.Core;
using me.cqp.luohuaming.NovelAI.Core.Domain;
using me.cqp.luohuaming.NovelAI.Sdk.Cqp;
using me.cqp.luohuaming.NovelAI.Sdk.Cqp.Enum;
using me.cqp.luohuaming.NovelAI.Sdk.Cqp.EventArgs;
using me.cqp.luohuaming.NovelAI.Sdk.Cqp.Interface;
using me.cqp.luohuaming.NovelAI.Sdk.Cqp.Expand;
using me.cqp.luohuaming.NovelAI.Sdk.Cqp.Model;
using Unity;
using Unity.Injection;

namespace me.cqp.luohuaming.NovelAI.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.NovelAI";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.NovelAI", 1, 9, "水银AI作画机", "1.0.0", 1, "落花茗", "Stable-diffusion-webui接口调用", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.NovelAI", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.NovelAI", AppData.CQLog);	
